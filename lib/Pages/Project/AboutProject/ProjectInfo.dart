...
@override
  ...
  Widget apartmentStructure(String projectName,BuildContext context) {
    ...
    List<List<String>> floorlistData  =List();
    floorlistData.clear();
          ...
          if(snapshot.hasData){
           ...
           // return  Text();
            return CarouselSlider.builder(
                ...
                itemCount: snapshot.data.length,
                itemBuilder: (BuildContext context,index){
                  double listLength = snapshot.data[index].cusList.length / int.parse(snapshot.data.last.cusList.last.id.substring(snapshot.data.last.cusList.last.id.length-1));





                  int flatPerFloor =int.parse(snapshot.data[index].cusList.last.id.substring(snapshot.data.last.cusList.last.id.length-1));
                //  print("flatPerFloor = ${flatPerFloor}");
                  int numberFlorr = listLength.toInt();
                  ...
                  int end =flatPerFloor;
                    for( int i=0;i<numberFlorr;i++){
                      ...
                      for(int j = start; j<end; j++){
                        innerdata.add(snapshot.data[index].cusList[j].id);
                      ...
                      children: [
                        ...
                        Expanded(
                          child: ListView.builder(
                              itemCount: floorlistData.length,

                              itemBuilder: (context,indexFloor){
                                return Text(floorlistData[indexFloor].toString());
                              }),
                        ),
                        ...
                        //         itemCount: snapshot.data[index].cusList.length,
                        //         gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(
                        //           crossAxisCount: int.parse(snapshot.data.last.cusList.last.id.substring(snapshot.data.last.cusList.last.id.length-1))  >2?
                        //           (int.parse(snapshot.data.last.cusList.last.id.substring(snapshot.data.last.cusList.last.id.length-1))/2).toInt() :2,
                        //
                        //           //int.parse(snapshot.data.last.cusList.last.id.substring(snapshot.data.last.cusList.last.id.length-1)),
                        //
